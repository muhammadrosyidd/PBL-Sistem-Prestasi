...
// Modified SQL query to include presma and dospem tables
$sql = "SELECT 
    p.*, 
    tl.nama_tingkat, 
    pr.nama_peringkat, 
    ...
    d.nomor_surat_tugas,
    STRING_AGG(m.nama_depan + ' ' + m.nama_belakang + ' (' + pm.nama_peran + ')', ', ') AS mahasiswa,
    STRING_AGG(do.nama + ' (' + pd.nama_peran + ')', ', ') AS dosen
FROM prestasi p
JOIN tingkatLomba tl ON p.tingkat_lomba_id = tl.tingkat_lomba_id
JOIN peringkat pr ON p.peringkat_id = pr.peringkat_id
LEFT JOIN dokumen d ON p.dokumen_id = d.dokumen_id
LEFT JOIN presma ps ON p.prestasi_id = ps.idpres
LEFT JOIN mahasiswa m ON ps.nim = m.nim
LEFT JOIN peran_mahasiswa pm ON ps.peran_mahasiswa_id = pm.peran_mahasiswa_id
LEFT JOIN dospem dp ON p.prestasi_id = dp.idpres
LEFT JOIN dosen do ON dp.dosen_id = do.dosen_id
LEFT JOIN peran_dosen pd ON dp.peran_dosen_id = pd.peran_dosen_id
GROUP BY p.prestasi_id, p.idpres, p.judul, p.tempat, p.link_kompetisi, p.tanggal_mulai, p.tanggal_akhir, p.jumlah_peserta, p.kategori_id, p.tingkat_lomba_id, p.peringkat_id, p.dokumen_id, p.verifikasi_status, p.tanggal_input, tl.nama_tingkat, pr.nama_peringkat, d.surat_tugas, d.tanggal_surat_tugas, d.nomor_surat_tugas;";
                    ...
                    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)): ?>
                      ...
                      <tr id="details-<?php echo $row['prestasi_id']; ?>" style="display:none;">
                        <td colspan="7" style="padding: 1rem; font-size: 12px;">
                          ...
                          <b>Mahasiswa:</b> <?php echo htmlspecialchars($row['mahasiswa'] ?? 'Tidak ada data'); ?><br>
                          <b>Dosen:</b> <?php echo htmlspecialchars($row['dosen'] ?? 'Tidak ada data'); ?><br>
